...
for i in 1:w*h
    if ProblemDefinition.id_to_location(i, w)[1] <= 5 && ProblemDefinition.id_to_location(i, w)[2] <= 5
        uwinds[:, i] = [45, 10]
    elseif ProblemDefinition.id_to_location(i, w)[1] > 5 && ProblemDefinition.id_to_location(i, w)[2] <= 5
        uwinds[:, i] = [180, 15]
    elseif ProblemDefinition.id_to_location(i, w)[1] <= 5 && ProblemDefinition.id_to_location(i, w)[2] > 5
        uwinds[:, i] = [90, 15]
    elseif ProblemDefinition.id_to_location(i, w)[1] > 5 && ProblemDefinition.id_to_location(i, w)[2] > 5
...
s1 = ProblemDefinition.RoutingProblem(w, h, uwinds, 100, 2500, 6.4, 1, 1, 15, 10)
Astaryes = false
@elapsed for i in 1:20
    ...
    open("results_scenario3"*tag*".csv","a") do f
